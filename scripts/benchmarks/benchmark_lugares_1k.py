...
CSV_PATH = ROOT_DIR / "scripts" / "benchmarks" / "generated" / "lugares_1k.csv"
RESULTS_DIR = Path(__file__).resolve().parent / "results"
RESULTS_DIR.mkdir(exist_ok=True)

MINI_TABLE = "lugares_1k"
POSTGRES_TABLE = "bench_lugares_1k"
...
TEST_CASES = [
    {
        "name": "buscar_id_500",
        "mini_sql": f"SELECT * FROM {MINI_TABLE} WHERE id = 500;",
        "pg_sql": f"SELECT * FROM {POSTGRES_TABLE} WHERE id = 500;",
    },
    {
        "name": "buscar_id_inexistente_999999",
        "mini_sql": f"SELECT * FROM {MINI_TABLE} WHERE id = 999999;",
        "pg_sql": f"SELECT * FROM {POSTGRES_TABLE} WHERE id = 999999;",
    },
    {
        "name": "rango_id_100_200",
        "mini_sql": f"SELECT * FROM {MINI_TABLE} WHERE id BETWEEN 100 AND 200;",
        "pg_sql": f"SELECT * FROM {POSTGRES_TABLE} WHERE id BETWEEN 100 AND 200;",
    },
]
...
def save_results(rows: list[dict]):
    results_file = RESULTS_DIR / "benchmark_lugares_1k_results.csv"
